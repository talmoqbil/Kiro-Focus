...
- **Anonymous User ID**: A unique identifier generated using crypto.randomUUID() stored in localStorage with key 'nimbus-user-id' to identify users without authentication
...
5. WHEN displaying agent messages, THEN the Agent System SHALL enforce a rate limit of maximum 30 agent messages per hour to prevent spam while allowing reasonable interaction.
...
1. WHEN the application loads for the first time, THEN the Cloud State System SHALL generate a unique anonymous user ID using crypto.randomUUID() and store it in localStorage with key 'nimbus-user-id'.
